...
import type {DailyHours, Holiday, OpeningHoursSettings, Role, Settings, SpecialPeriod} from '../types';
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                {users.map(user => (
                    <li key={user.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-md">
                        ...
                        <div className="space-x-2">
                            ...
                            {user.role !== 'admin' &&
                                <button onClick={() => window.confirm(`Supprimer ${user.name}?`) && deleteUser(user.id)}
            ...
            <button onClick={handleAddUser}
                    className="mt-4 px-4 py-2 bg-brand-primary text-white rounded-md hover:bg-sky-700">Ajouter un
                utilisateur
            </button>
        </div>
    );
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <input type="number" min="1" max="28" value={localSettings.reminderStartDay}
                           onChange={e => setLocalSettings({
                               ...localSettings,
                               reminderStartDay: parseInt(e.target.value)
                           })} className="mt-1 w-full p-2 border border-gray-300 rounded-md"/>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700">Créneaux obligatoires / mois</label>
                    <input type="number" min="0" value={localSettings.minSlotsRequired}
                           onChange={e => setLocalSettings({
                               ...localSettings,
                               minSlotsRequired: parseInt(e.target.value)
                           })} className="mt-1 w-full p-2 border border-gray-300 rounded-md"/>
                </div>
            </div>
            <button onClick={handleSave}
                    className="mt-6 px-4 py-2 bg-brand-primary text-white rounded-md hover:bg-sky-700">Sauvegarder
            </button>
        </div>
    );
};
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <form onSubmit={handleAddEvent} className="space-y-4 mb-6">
                <input name="title" placeholder="Titre de l'évènement"
                ...
                <button type="submit"
                        className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Ajouter l'évènement
                </button>
            </form>
                ...
                {events.slice().sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime()).map(event => (
                    <li key={event.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-md">
                        ...
                        <button onClick={() => deleteEvent(event.id)}
                                className="p-2 text-red-600 hover:text-red-800 rounded-full hover:bg-red-100"><TrashIcon
                ...
                <ul className="space-y-3">
                    {reminderList.map(user => (
                        <li key={user.id} className="flex justify-between items-center bg-amber-50 p-3 rounded-md">
                            ...
                            <button
                                className="flex items-center px-3 py-1 bg-amber-500 text-white rounded-md text-sm hover:bg-amber-600">
...
const DayHoursEditor: React.FC<{
    day: DailyHours,
    onUpdate: (field: keyof DailyHours, value: any) => void,
    dayName: string
}> = ({day, onUpdate, dayName}) => {
    return (
        <div className="grid grid-cols-4 gap-2 items-center">
            <label className="font-medium text-gray-700">{dayName}</label>
            <div className="flex items-center">
                <input type="checkbox" checked={day.isOpen} onChange={e => onUpdate('isOpen', e.target.checked)}
                       className="h-4 w-4 text-brand-primary border-gray-300 rounded focus:ring-brand-primary"/>
                <span className="ml-2 text-sm">{day.isOpen ? 'Ouvert' : 'Fermé'}</span>
            </div>
            <input type="time" step="1800" value={day.start} disabled={!day.isOpen}
                   onChange={e => onUpdate('start', e.target.value)}
                   className="p-1 border border-gray-300 rounded-md disabled:bg-gray-100"/>
            <input type="time" step="1800" value={day.end} disabled={!day.isOpen}
...
const OpeningHoursManagement: React.FC = () => {
    ...
    const handleDefaultHoursChange = (dayOfWeek: number, field: keyof DailyHours, value: any) => {
        setLocalHours(prev => ({
            ...prev,
            defaultHours: prev.defaultHours.map(d => d.dayOfWeek === dayOfWeek ? {...d, [field]: value} : d)
        }));
    }

    const handlePeriodHoursChange = (periodId: string, dayOfWeek: number, field: keyof DailyHours, value: any) => {
        setLocalHours(prev => ({
            ...prev,
            specialPeriods: prev.specialPeriods.map(p => p.id === periodId ? {
                ...p,
                hours: p.hours.map(d => d.dayOfWeek === dayOfWeek ? {...d, [field]: value} : d)
            } : p)
        }));
    }
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg space-y-8">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Définissez les horaires pour les mois à venir, les périodes
                        spéciales et les jours de fermeture.</p>
                </div>
                <button onClick={handleSave}
                        className="px-6 py-2 bg-brand-primary text-white rounded-md hover:bg-sky-700 font-semibold">Sauvegarder
                    les Horaires
                </button>
            </div>
            ...
            <div className="border border-gray-200 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                    {localHours.defaultHours.sort((a, b) => a.dayOfWeek === 0 ? 1 : b.dayOfWeek === 0 ? -1 : a.dayOfWeek - b.dayOfWeek) // Sunday last
                        .map(day => <DayHoursEditor key={day.dayOfWeek} day={day}
                                                    onUpdate={(field, value) => handleDefaultHoursChange(day.dayOfWeek, field, value)}
                ...
                <div className="space-y-4">
                        ...
                        <div key={period.id} className="bg-sky-50 p-4 rounded-md">
                            ...
                            <div className="space-y-2">
                                {period.hours.sort((a, b) => a.dayOfWeek === 0 ? 1 : b.dayOfWeek === 0 ? -1 : a.dayOfWeek - b.dayOfWeek).map(day =>
                                    <DayHoursEditor key={day.dayOfWeek} day={day}
                                                    onUpdate={(field, value) => handlePeriodHoursChange(period.id, day.dayOfWeek, field, value)}
                                                    dayName={weekdays[day.dayOfWeek]}/>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex items-end gap-4 bg-gray-50 p-3 rounded-md">
                        <input value={newPeriod.name} onChange={e => setNewPeriod({...newPeriod, name: e.target.value})}
                               placeholder="Nom (ex: Vacances d'été)"
                               className="flex-grow p-2 border border-gray-300 rounded-md"/>
                        <input value={newPeriod.start}
                               onChange={e => setNewPeriod({...newPeriod, start: e.target.value})} type="date"
                               className="p-2 border border-gray-300 rounded-md"/>
                        <input value={newPeriod.end} onChange={e => setNewPeriod({...newPeriod, end: e.target.value})}
                               type="date" className="p-2 border border-gray-300 rounded-md"/>
                        <button onClick={handleAddPeriod}
                                className="p-2 bg-green-600 text-white rounded-md hover:bg-green-700"><PlusIcon
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-end gap-2 bg-gray-50 p-3 rounded-md">
                        <input value={newHoliday.name}
                               ...
                               className="flex-grow p-2 border border-gray-300 rounded-md"/>
                        <input value={newHoliday.date}
                               onChange={e => setNewHoliday({...newHoliday, date: e.target.value})} type="date"
                               className="p-2 border border-gray-300 rounded-md"/>
