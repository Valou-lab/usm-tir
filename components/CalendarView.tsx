...
import { getDay, isSameDay, addMonths, subMonths, getMonthName } from '../utils/dateUtils';
import type { Slot, Event } from '../types';
import { ChevronLeftIcon, ChevronRightIcon, PlusIcon, WandIcon, TrashIcon, PencilIcon } from './Icons';
...
const CalendarHeader: React.FC<{
  ...
  <div className="flex items-center justify-between mb-6">
    ...
    <div className="text-center">
        ...
        <button onClick={onShowTemplateModal} className="mt-1 inline-flex items-center px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-sm font-medium hover:bg-amber-200 transition-colors">
...
const DayCell: React.FC<{ 
...
}> = ({ day, slots, events, onAddSlot, onEditSlot }) => {
    const context = useContext(AppContext);
    const { currentUser, deleteSlot } = context!;
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm p-3 flex flex-col min-h-[140px] border border-gray-200 hover:border-brand-primary transition-all duration-300 group">
            <div className="flex justify-between items-center">
                <span className={`font-semibold ${isToday ? 'text-brand-primary' : 'text-gray-600'}`}>
                    {day.getDate()}
                </span>
                <button onClick={() => onAddSlot(day)} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full bg-brand-secondary hover:bg-sky-200">
                    <PlusIcon className="w-5 h-5 text-brand-primary" />
                </button>
            </div>
            <div className="mt-2 space-y-1 overflow-y-auto flex-grow">
                {events.map(event => (
                    <div key={event.id} className="text-xs p-1.5 rounded-md bg-red-100 text-red-800 font-medium w-full text-left truncate">
                        {event.allDay ? event.title : `${new Date(event.start).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })} ${event.title}`}
                    </div>
                ))}
                {sortedSlots.map(slot => {
                    const isCurrentUserSlot = slot.userId === currentUser?.id;
                    return (
                        <div key={slot.id} className={`text-xs p-1.5 rounded-md w-full text-left ${isCurrentUserSlot ? 'bg-sky-100 text-sky-800' : 'bg-gray-100 text-gray-700'}`}>
                            ...
                            <div>
                             ...
                             {isCurrentUserSlot && (
                                <div className="flex items-center justify-end space-x-2 mt-1">
                                    <button onClick={() => onEditSlot(slot)} className="text-blue-600 hover:text-blue-800"><PencilIcon className="w-4 h-4"/></button>
                                    <button onClick={() => window.confirm('Êtes-vous sûr de vouloir supprimer ce créneau ?') && deleteSlot(slot.id)} className="text-red-600 hover:text-red-800"><TrashIcon className="w-4 h-4"/></button>
                                </div>
                            )}
                        </div>
                    );
